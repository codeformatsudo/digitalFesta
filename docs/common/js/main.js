...
/*
	...
	csvToArray('data/info.csv', function (data) {
		...
		switch (colorData) {
			...
			case '黄色':
				...
				break;
			case '青':
				document.querySelector('body').classList.add('is_blue');
				break;
		}
	...
	csvToArray('data/caution.csv', function (data) {
		...
		if (dataLen - 1 > 0) {
			...
			document.querySelector('.event').appendChild(cautionArea);
	...
	*/
	...
	csvToArray('data/sponsors.csv', function (data) {
		...
		//主催
		createSection(data[1][0], 'sponsors_organizer', '出展協力', sponsors);

		//フッター
		var footer = document.querySelector('footer');
		createDiv(data[1][1], footer);
	});
